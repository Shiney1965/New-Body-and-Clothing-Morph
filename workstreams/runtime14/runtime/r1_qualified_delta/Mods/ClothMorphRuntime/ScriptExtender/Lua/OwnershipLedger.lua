...
local function restoreProvenOriginals(keep,record)
    local restored={}
    local original=record.OriginalVisuals or {}
    for index,guid in ipairs(original) do
        if (record.RemovedOriginalVisuals or {})[guid]==true then
            local present=false
            for _,live in ipairs(keep) do if lower(live)==lower(guid) then present=true end end
            if not present then
                local position=#keep+1
                for nextIndex=index+1,#original do
                    local found
                    for liveIndex,live in ipairs(keep) do
                        if lower(live)==lower(original[nextIndex]) then found=liveIndex;break end
                    end
                    if found then position=found;break end
                end
                table.insert(keep,position,guid)
            end
            restored[#restored+1]=guid
        end
    end
    return restored
end
...
local function doRestore(charGuid, record, deps, options)
    ...
    if deps.isCharacterAvailable ~= nil and deps.isCharacterAvailable(charGuid) ~= true then
        ...
        record.RestoreFailures = { "character-unavailable" }
        if options.makeExternal then record.Choice = "external" end
        return { status = "pending", failureCodes = record.RestoreFailures }
    end
    ...
    if record.CvGuid==nil or liveCv==nil or lower(record.CvGuid)~=lower(liveCv) then
        ...
        record.RestoreFailures={"BODY_CV_MISMATCH"}
    ...
    if type(currentVisuals) ~= "table" then
        ...
        local restoredOriginals=restoreProvenOriginals(keep,record)
        local writeOk = true
