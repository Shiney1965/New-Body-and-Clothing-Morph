...
class CandidateContract:
    """Geometry-specific inputs; all numeric acceptance thresholds stay fixed in code."""
    ...
    def __post_init__(self) -> None:
        if not set(self.constraints.active_ids).issubset(self.roi.active_ids):
            raise ValueError("surface constraints must belong to the ROI active set")
...
def _source_arrays(source: object) -> tuple[np.ndarray, np.ndarray, str | None]:
    ...
    return positions, faces, semantics
...
def solve_coherent_field(
    ...
    if not set(constraints.active_ids).issubset(roi.active_ids):
        raise ValueError("constraints contain a vertex outside the ROI active set")
...
def evaluate_candidate(
    ...
    base, faces, source_semantics = _source_arrays(source)
    topology_equal = candidate.source_face_indices_sha256 == _face_digest(faces)
