...
local R1Runtime
local R1Capability
local function R1CanWrite(_entryPoint, char)
    if R1Runtime == nil then return true end
    local accepted = R1Runtime:WriteGate(R1Capability or "managed", char, function() return true end)
    return accepted == true
end

local function R1WithCapability(capability, fn)
    local previous = R1Capability
    R1Capability = capability
...
local function IsTrustedSchema6BodyOriginal(guid, cvGuid)
    ...
    local currentBody
    pcall(function()
        local characterVisual = Ext.Resource.Get(cvGuid, "CharacterVisual")
        currentBody = tostring(characterVisual.VisualSet.BodySetVisual)
    end)
    if currentBody == nil or currentBody == "" or currentBody == "nil"
        or currentBody:lower() ~= needle then return false end
    return true
end
...
local function ResolveNetTarget(data, user)
    ...
    -- Party-member tolerance for legacy per-character commands. Global tattoo
    -- policy changes bypass this path and use strict component-backed host
...
local function NormalizeNetUserId(user)
...
local function SetBaseBody(char, choice, targetVr)
    ...
    if cur == target then
        BaseWrites[cv] = { orig = orig, cur = cur, char = char }
        return true, "already"
    end
    if bw ~= nil and bw.char ~= nil and bw.char ~= char then
        Warn(("SetBaseBody: CV %s is SHARED (%s vs %s) - last write wins")
            :format(cv, tostring(bw.char), tostring(char)))
    end
    local okW = pcall(function()
        local r = GetCVRes(cv)
    ...
    BaseWrites[cv] = { orig = orig, cur = target, char = char }
...
local function RestoreAllBaseWrites(reason)
    local n = 0
    for cv, bw in pairs(BaseWrites) do
        if not R1CanWrite("restore-base-write", bw.char) then goto continue end
...
local function SetBodyTattooPolicy(policy, source)
    if not R1CanWrite("mcm-tattoo", nil) then return false end
...
R1Runtime = R1Foundation.Install(MOD, EnsurePV(), {
    ...
    applyManaged = function(char, _record, choice)
        return R1WithCapability("ordinary_reenable", function()
            return SetDesiredBodyManaged(char, choice)
        end)
    end,
    ...
    writeEquipRace = function(char, guid)
        ...
        if ok then pcall(function() EquipRace.RefreshEquipment(char) end) end
        return ok
    end,
...
local function Cmd_ApplyCcsv(_cmd, ccsv, charArg)
    Warn("!cm_applyccsv is disabled in schema 7; raw CCSVs cannot establish trusted ownership")
    return false
end
...
local function Cmd_SetERace(_cmd, guid, charArg)
    Warn("!cm_seterace is disabled in schema 7; arbitrary values cannot become trusted originals")
    return false
end
...
local function Cmd_Master(_cmd, arg)
    ...
    local result = MOD.SetMasterEnabled(value == "on")
...
local MCM_DEPS = { Log = Log, Warn = Warn, IsValid = Shared.IsValidBodyChoice,
                   ...
                   SetMasterEnabled = MOD.SetMasterEnabled,
...
pcall(function()
    Ext.Osiris.RegisterListener("LevelGameplayStarted", 2, "after", function(_level, _isEditor)
        ...
        pcall(function()
            R1WithCapability("maintenance_restore", function()
                RestoreAllBaseWrites("LevelGameplayStarted")
            end)
        end)
    ...
    Ext.Osiris.RegisterListener("SavegameLoaded", 0, "after", function()
        ...
        pcall(function()
            R1WithCapability("maintenance_restore", function()
                RestoreAllBaseWrites("SavegameLoaded")
            end)
        end)
