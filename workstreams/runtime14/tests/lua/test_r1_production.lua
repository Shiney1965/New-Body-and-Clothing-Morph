...
test('late character is pending on master disable and restored on lifecycle',function()
    local w=F.Boot(root)
    local entity=w.entities[F.ids.a];w.entities[F.ids.a]=nil
    local _,summary=w.mod.SetMasterEnabled(false,'test-server')
    eq(w.state.Bodies[F.ids.a].RestoreState,'pending','late record state')
    assert(type(summary.pendingGuids)=='table' and summary.pendingGuids[1]==F.ids.a,'pending GUID missing')
    w.entities[F.ids.a]=entity
    w.fire('SavegameLoaded')
    eq(w.state.Bodies[F.ids.a].RestoreState,'clean','late restored')
    eq(w.state.PassThroughRestoreComplete,true,'complete only after late restore')
end)
