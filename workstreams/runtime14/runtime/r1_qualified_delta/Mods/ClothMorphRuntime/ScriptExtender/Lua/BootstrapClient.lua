...
local function ForwardForTarget(cmd, arg, explicitTarget)
    ...
    if target == nil then
        ...
        Warn(("'%s': no controlled character resolved; request not sent."):format(tostring(cmd)))
        return
    end
...
Ext.RegisterConsoleCommand("cm_master", function(_cmd, value)
    ...
    if value ~= "on" and value ~= "off" and value ~= "status" then Warn("!cm_master <on|off|status>"); return end
...
local BODY_LABEL = { vanilla = "Vanilla", sbbf = "SBBF", bcb = "BCB", external = "External / Pass-through" }
pcall(function()
